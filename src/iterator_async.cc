...
namespace kududown {
  ...
  NextWorker::NextWorker(Iterator* iterator, Nan::Callback *callback,
                         void (*localCallback)(Iterator*))
      : AsyncWorker(NULL, callback), iterator(iterator),
        localCallback(localCallback), ok(true) {
  }
  ...
  NextWorker::HandleOKCallback() {
    ...
    for (idx = 0; idx < result.size(); ++idx) {
      ...
      if (iterator->keyAsBuffer) {
        ...
        returnKey =
            Nan::New<v8::String>((char*) key.data(), key.size()).ToLocalChecked();
      }
      ...
      returnArray->Set(
          Nan::New<v8::Integer>(static_cast<int>(arraySize - idx * 2 - 1)),
          returnKey);
      returnArray->Set(
          Nan::New<v8::Integer>(static_cast<int>(arraySize - idx * 2 - 2)),
          returnValue);
    }
